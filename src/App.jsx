...
import { ClosetImportFlow } from "./import-flow.jsx";
import { OptimizedImage } from "./OptimizedImage.jsx";

const STORAGE_KEY = "closet-edits-v1";
const DELETED_STORAGE_KEY = "closet-deleted-v1";

const TYPES = [
  { id: "all", label: "すべて" },
  { id: "upperbody", label: "トップス", singular: "トップス" },
  { id: "wholebody_up", label: "アウター", singular: "アウター" },
  { id: "lowerbody", label: "ボトムス", singular: "ボトムス" },
  { id: "accessories_up", label: "小物", singular: "小物" },
  { id: "shoes", label: "シューズ", singular: "シューズ" },
];
...
function GalleryItem({ item, selected, onOpen }) {
  const type = TYPE_MAP[item.part]?.singular || "アイテム";
    ...
    <div className="tag-editor">
      <div className="editable-tags">
        {tags.map((tag) => (
          <span className="editable-tag" key={tag}>
            {tag}
            <button type="button" onClick={() => onChange(tags.filter((existing) => existing !== tag))} aria-label={`${tag}を削除`}>
      ...
      <div className="tag-input-row">
        <input
          ...
          aria-label="特徴タグを追加"
        />
        <button type="button" onClick={addTag} disabled={!input.trim()} aria-label="特徴を追加">
...
function ColorControl({ label, field, value, palette, onChange, sampling, setSampling, optional = false, onClear, onAdd }) {
  if (optional && !value) {
    return (
      <div className="color-slot empty-color-slot">
        ...
        <p>はっきりしたサブカラーは見つかりませんでした。</p>
        <button className="add-secondary-button" type="button" onClick={onAdd}>サブカラーを追加</button>
      </div>
    );
  }
  ...
  return (
    <div className="color-slot">
      <div className="color-slot-heading">
        <span>{label}</span>
        {optional && <button type="button" onClick={onClear}>削除</button>}
      </div>
      ...
      <div className="palette" aria-label={`${label}の画像候補`}>
        {palette.map((color) => (
          <button
            type="button"
            key={color}
      ...
      >
        {sampling === field ? "ピックをやめる" : `画像から${label}をピック`}
      </button>
    </div>
  );
}
    ...
    <div className="item-editor">
      <label className="field">
        ...
        <input
          ...
          placeholder={TYPE_MAP[draft.part]?.singular || "アイテム"}
        />
      </label>
      ...
      <fieldset className="color-field">
        ...
        <p className="color-help" aria-live="polite">{sampling ? `服の上をタップして${sampling === "secondary" ? "サブ" : "メイン"}カラーを拾えます。` : sampleStatus || "メインカラーは画像から自動で拾います。サブは色がはっきり違うときだけ提案します。"}</p>
      </fieldset>
...
function ItemViewer({ item, onClose, onSave, onDelete }) {
  ...
  const handleImageClick = (event) => {
    ...
    if (!color) {
      setSampleStatus("そこは透明です。服の上をタップしてみてください。");
      return;
    }
    ...
    setSampleStatus(`${sampling === "secondary" ? "サブ" : "メイン"}カラーに ${color} をセットしました。`);
    setSampling(null);
  };
  ...
  const garmentArtwork = (
    ...
    >
      ...
      />
      {sampling && <span className="sample-hint">服をタップして色を拾う</span>}
    </div>
  );
  ...
  return (
    ...
    <aside className={`viewer editing${hasModeledImage ? " has-modeled-image" : ""}${shaking ? " shake" : ""}`} role="dialog" aria-modal="true" aria-label="選択中のアイテム">
      <button className="viewer-icon-close" type="button" onClick={requestClose} aria-label="閉じる" ref={closeButtonRef}>
      ...
      <div className="viewer-details editing">
        ...
        {closeBlocked && <p className="unsaved-notice" role="status">閉じる前に保存かキャンセルをしてください。</p>}

        <div className="viewer-actions">
          <button className="delete-button" type="button" onClick={() => onDelete(item.id)}>
            <Trash size={15} weight="regular" aria-hidden="true" /> 削除
          </button>
          <span className="action-spacer" />
          <button className="secondary-button" type="button" onClick={cancelEditing}>キャンセル</button>
...
export function App() {
  ...
  useEffect(() => {
    fetch("/api/import/wardrobe", { cache: "no-store" })
      .then((response) => {
        if (!response.ok) throw new Error("クローゼットを読み込めませんでした。");
  ...
  const deleteItem = async (id) => {
    if (id.startsWith("import-")) {
      try {
        ...
        if (!response.ok && response.status !== 404) throw new Error("インポートしたアイテムを削除できませんでした。");
      } catch (requestError) {
  ...
  return (
    <div className={`app-shell${selectedItem ? " has-selection" : ""}`}>
      <main className="gallery-pane">
        <header className="gallery-header">
          <div className="brand-block">
            <p className="brand-mark">クローゼット</p>
            <p className="brand-tagline">写真をぽん、とかわいく整理</p>
          </div>
          <div className="gallery-meta-row">
            <p className="piece-count">{items.length} 着</p>
          </div>
          <nav className="category-nav" aria-label="カテゴリで絞り込み">
            {TYPES.map((type) => (
              <button
                key={type.id}
                type="button"
        ...
        {!error && loading && <p className="status">よみこみ中…</p>}
        {!error && !loading && !items.length && <p className="status empty">写真をドロップ・貼り付け・追加して、最初の一着を入れよう。</p>}

        {!!items.length && (
          <section className="gallery-grid" aria-label={`${TYPE_MAP[activeType]?.label || "すべて"}のアイテム`}>
      ...
      <ClosetImportFlow onGarmentApproved={addImportedItem} onModeledApproved={attachImportedModeledImage} />
    </div>
  );
}
